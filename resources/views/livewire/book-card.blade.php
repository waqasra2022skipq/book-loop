...
<div class="group relative bg-white rounded-2xl shadow-lg border border-blue-100 hover:shadow-xl hover:border-blue-300 transition-all duration-300 overflow-hidden flex flex-col">
    <!-- Book Cover -->
    <a href="{{ route('books.instance', $instance->id) }}" class="relative overflow-hidden block rounded-t-2xl">
        ...
        @else
            <div class="w-full h-48 sm:h-56 bg-gradient-to-br from-blue-50 to-purple-50 flex items-center justify-center rounded-t-2xl">
                <svg class="w-12 sm:w-16 h-12 sm:h-16 text-blue-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="absolute top-2 sm:top-3 right-2 sm:right-3">
            <span class="px-2 sm:px-3 py-1 text-xs font-semibold rounded-full shadow-sm border 
                {{ $instance->status === 'available' ? 'bg-green-100 text-green-700 border-green-200' : 
                   ($instance->status === 'requested' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-gray-100 text-gray-700 border-gray-200') }}">
    ...
    <div class="p-4 sm:p-6 space-y-3 sm:space-y-4 flex-1 flex flex-col">
        ...
        <a href="{{ route('books.instance', $instance->id) }}">
            <h3 class="text-lg sm:text-xl font-bold text-blue-800 line-clamp-2 group-hover:text-blue-600 transition-colors">
                {{ $instance->book->title }}
            </h3>
        </a>
        ...
        @if($instance->condition_notes)
            <div class="bg-amber-50 border border-amber-200 rounded-lg p-2 sm:p-3 mt-2">
