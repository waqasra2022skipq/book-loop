...
<div class="group relative bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-2xl hover:border-blue-200 transition-all duration-300 overflow-hidden">
    ...
    <div class="p-4 sm:p-6 space-y-3 sm:space-y-4">
        ...
        <a href="{{ route('books.instance', $instance->id) }}">
            <h3 class="text-lg sm:text-xl font-bold text-gray-900 line-clamp-2 group-hover:text-blue-600 transition-colors">
                {{ $instance->book->title }}
            </h3>
        </a>
        ...
        <!-- Action Buttons -->
        <div class="flex flex-col gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-gray-100">
            <a href="{{ route('books.instance', $instance->id) }}" 
               class="flex-1 text-center px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors duration-200 flex items-center justify-center">
            ...
            @if($instance->status === 'available')
                <a href="{{ route('books.instance.request', ['bookInstance' => $instance->id]) }}" 
                   class="flex-1 text-center px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-200 transform hover:scale-105 flex items-center justify-center">
                    <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <button disabled 
                        class="flex-1 text-center px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-medium text-gray-400 bg-gray-100 rounded-lg cursor-not-allowed flex items-center justify-center">
                    <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
