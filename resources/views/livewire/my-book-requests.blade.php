    ...
    <div>
        @if ($tab === 'pending')
            @foreach ($pendingRequests as $request)
                <div
                    class="flex flex-col sm:flex-row items-start sm:items-center bg-white rounded shadow p-4 border mb-3">
                    <div class="w-full sm:w-20 h-28 flex-shrink-0 mb-2 sm:mb-0 sm:mr-4">
                        @if ($request->book->cover_image)
                            <img src="{{ asset('storage/' . $request->book->cover_image) }}" alt="Book Cover"
                                class="w-full h-full object-cover rounded">
                        @else
                            <div
                                class="w-full h-full bg-gray-200 flex items-center justify-center rounded text-gray-400">
                                No Image</div>
                        @endif
                    </div>
                    <div class="flex-1 w-full">
                        <div class="font-bold text-lg">{{ $request->book->title }}</div>
                        <div class="text-sm text-gray-600">Requested by:
                            @if ($request->requester)
                                <a href="{{ route('users.profile', $request->requester->id) }}"
                                    class="font-semibold">{{ $request->requester->name }}</a>
                            @else
                                <span class="font-semibold">{{ $request->name }}</span>
                            @endif
                        </div>
                        <div class="text-sm text-gray-600">Email: {{ $request->email }}</div>
                        <div class="text-sm text-gray-600">Message: <span class="italic">{{ $request->message }}</span>
                        </div>
                        <div class="mt-2">
                    ...
                    <div class="flex flex-row sm:flex-col gap-2 mt-2 sm:mt-0 sm:ml-4">
                        <button wire:click="accept({{ $request->id }})"
                            class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700">Accept</button>
