    ...
    <div class="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
                ...
                <div class="flex flex-col lg:flex-row">
                    ...
                    <div class="lg:w-80 p-6 sm:p-8 bg-gradient-to-br from-blue-50 to-indigo-50 flex justify-center">
                        <div class="relative group">
                            <div class="absolute -inset-2 bg-gradient-to-r from-blue-400 via-purple-400 to-indigo-400 rounded-2xl opacity-20 group-hover:opacity-30 transition-opacity blur"></div>
                            <div class="relative w-48 h-64 sm:w-52 sm:h-72 bg-white rounded-xl shadow-2xl overflow-hidden transform group-hover:scale-105 transition-transform duration-300">
                                @if ($book->cover_image)
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex flex-wrap gap-3">
                                <div class="flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-full">
                                    ...
                                    <span class="text-sm font-semibold text-blue-700">{{ $book->genre->name ?? 'General' }}</span>
                                </div>
                                ...
                                <div class="flex items-center gap-2 bg-amber-50 px-4 py-2 rounded-full">
                                    ...
                                    <span class="text-sm font-semibold text-amber-700">{{ $book->average_rating() ?? '0.00' }}/5</span>
                                ...
                                <div class="flex items-center gap-2 bg-green-50 px-4 py-2 rounded-full">
                                    <svg class="w-4 h-4 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-8.293l-3-3a1 1 0 00-1.414 1.414L10.586 9.5H6a1 1 0 100 2h4.586l-1.293 1.293a1 1 0 101.414 1.414l3-3a1 1 0 000-1.414z" clip-rule="evenodd"/>
                                    </svg>
                                    <span class="text-sm font-semibold text-green-700">{{ $instances->count() }} {{ $instances->count() === 1 ? 'copy' : 'copies' }} available</span>
                                </div>
                            </div>
                            ...
                            @if($book->isbn)
                            <div class="pt-4">
                                <div class="text-sm text-gray-500">ISBN: <span class="font-mono text-gray-700">{{ $book->isbn }}</span></div>
                            </div>
                            @endif
                        </div>
                    </div>
        ...
        <div x-data="{ tab: 'details' }" class="space-y-6">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-1">
                <div class="flex flex-wrap gap-1">
                    <button @click="tab = 'details'"
                        :class="tab === 'details' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:text-blue-600 hover:bg-blue-50'"
                    ...
                    <button @click="tab = 'summaries'"
                        :class="tab === 'summaries' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:text-blue-600 hover:bg-blue-50'"
                        class="flex-1 sm:flex-none px-6 py-3 font-semibold rounded-lg transition-all duration-200 flex items-center justify-center gap-2">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <button @click="tab = 'instances'"
                        :class="tab === 'instances' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:text-blue-600 hover:bg-blue-50'"
                        class="flex-1 sm:flex-none px-6 py-3 font-semibold rounded-lg transition-all duration-200 flex items-center justify-center gap-2">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div x-show="tab === 'details'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" class="p-6 sm:p-8">
                    <div class="space-y-8">
                        <!-- Description Section -->
                        <div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                <div class="bg-gray-50 rounded-lg p-4">
                                    ...
                                    <dd class="text-lg font-semibold text-gray-900">{{ $book->genre->name ?? 'General' }}</dd>
                                </div>
                                ...
                                <div class="bg-gray-50 rounded-lg p-4">
                                    ...
                                    <dd class="text-lg font-mono font-semibold text-gray-900">{{ $book->isbn }}</dd>
                                </div>
                                @endif
                                ...
                                <div class="bg-gray-50 rounded-lg p-4">
                                    ...
                                    <dd class="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                        ...
                                        <div class="flex">
                                            @for($i = 1; $i <= 5; $i++)
                                                <svg class="w-4 h-4 {{ $i <= floor($book->average_rating() ?? 0) ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                </svg>
                                            @endfor
                                        </div>
                                ...
                                <div class="bg-gray-50 rounded-lg p-4">
                                    ...
                                    <dd class="text-lg font-semibold text-gray-900">{{ $book->total_ratings() }} {{ $book->total_ratings() === 1 ? 'review' : 'reviews' }}</dd>
                                </div>

                                <!-- Available Copies -->
                                <div class="bg-gray-50 rounded-lg p-4">
                                    <dt class="text-sm font-medium text-gray-500 mb-1">Available Copies</dt>
                                    <dd class="text-lg font-semibold text-gray-900">{{ $instances->count() }} {{ $instances->count() === 1 ? 'copy' : 'copies' }}</dd>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <!-- Summaries Tab -->
                <div x-show="tab === 'summaries'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" class="p-6 sm:p-8">
                    @livewire('book-summaries', ['bookId' => $book->id])
                </div>

                <!-- Enhanced Available Copies Tab -->
                <div x-show="tab === 'instances'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" class="p-6 sm:p-8">
                    <div class="space-y-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="bg-blue-50 px-3 py-1 rounded-full">
                                <span class="text-sm font-semibold text-blue-700">{{ $instances->count() }} {{ $instances->count() === 1 ? 'copy' : 'copies' }}</span>
                            </div>
                        </div>
                                        ...
                                        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-gray-100">
                                            <div class="flex items-center gap-3">
                                                <div class="w-10 h-10 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full flex items-center justify-center">
                                                    <span class="text-white font-semibold text-sm">{{ strtoupper(substr($instance->owner->name, 0, 1)) }}</span>
                                                </div>
                                                <div>
                                                    <h4 class="font-semibold text-gray-900">{{ $instance->owner->name }}</h4>
                                        ...
                                        <div class="p-6 space-y-4">
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="font-semibold text-gray-900">{{ $instance->city }}{{ $instance->address ? ', ' . $instance->address : '' }}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="font-semibold text-gray-900">{{ $instance->condition_notes ?: 'Good condition' }}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-3">
                                                <div class="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center">
                                                    <svg class="w-4 h-4 {{ $instance->status === 'available' ? 'text-green-600' : 'text-red-600' }}" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                                    </svg>
                                                </div>
                                                <div>
                                                    <p class="text-sm text-gray-500">Status</p>
                                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $instance->status === 'available' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
