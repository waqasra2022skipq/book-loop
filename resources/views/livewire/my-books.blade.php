                ...
                <div class="flex flex-col sm:block p-3 sm:p-4 border rounded-lg shadow bg-white">
                    @if ($instance->book->cover_image)
                        <img src="{{ asset('storage/' . $instance->book->cover_image) }}"
                            class="w-full h-40 sm:h-48 object-cover rounded mb-2 sm:mb-3" alt="Book cover">
                    @else
                        <div
                            class="w-full h-40 sm:h-48 bg-gray-100 flex items-center justify-center text-gray-400 rounded mb-2 sm:mb-3">
                            No Image
                        </div>
                    @endif
                    ...
                    <p class="text-xs sm:text-sm text-gray-500 mb-1 sm:mb-2">Status: <span
                            class="font-medium capitalize">{{ $instance->status }}</span></p>
                    @if ($instance->price)
                        <p class="text-xs sm:text-sm text-green-600 font-semibold mb-1 sm:mb-2">Price:
                            ${{ number_format($instance->price, 2) }}</p>
                    @endif
                    ...
                    <div class="flex flex-col sm:flex-row gap-1 sm:gap-2 mt-2">
                        ...
                        <button wire:click="delete({{ $instance->id }})"
                            ...
                            wire:confirm="Are you sure you want to delete this Book">Delete</button>
                    </div>
                </div>
            @endforeach
        </div>
